...
static void OutputLut_Init()
{
    ...
    for (int c = 0; c < 3; c++) {
        for (int v = 0; v < 256; v++) {
            // Чёрная точка по сырому входу (эта таблица — БЕЗ яркости): всё ниже
            // порога в чёрное. В outputBrightLut порог берётся уже после яркости —
            // там диммирование топит в чёрное больше остаточного шума.
...
void LedTask_Handler( void * pvParameters )
{
    ...
    while (1) {
        ...
        // Вывод через emit_frame: ремап (дизайн→факт калибровки) + яркость и
        // цветокоррекция одной таблицей outputBrightLut + пропуск ре-латча
        // неизменившегося кадра. lastRawFrame — всегда сырой дизайн-кадр,
        // коррекция только на выходе.
        emit_frame(lastRawFrame);
...
// яркость с цветокоррекцией одной таблицей (outputBrightLut: гамма/чёрная
// точка/баланс белого + вшитая яркость) → FRAME_BUFFER → SPI. source —
// дизайн-кадр (lastRawFrame при воспроизведении). remapTable == NULL —
// идентичность по геометрии (кадр как есть); outputBrightLut при нейтральных
// константах и яркости 255 — идентичность по цвету, вместе это прежний
